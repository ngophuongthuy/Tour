...
export default function Bac() {
  ...
  useEffect(() => {
    const fetchTours = async () => {
      try {
        ...
        setFilteredTours(response.data); // Initially set filteredTours to contain all tours
      } catch (error) {
  ...
  const handleSearch = () => {
    const filtered = tours.filter((tour) => {
      ...
      return tour.idTour === "MB" && matchesSearchTerm && matchesStartDate;
    });
    setFilteredTours(filtered);
    setCurrentPage(1); // Reset to first page when search is performed
  };

  // Calculate the current tours
  const indexOfLastTour = currentPage * toursPerPage;
  const indexOfFirstTour = indexOfLastTour - toursPerPage;
  const currentTours = filteredTours.slice(indexOfFirstTour, indexOfLastTour);

  const nextPage = () => {
    if (currentPage < Math.ceil(filteredTours.length / toursPerPage)) {
    ...
    <>
      ...
      <div className="flex justify-center space-x-5 mb-5">
        ...
        <button
          ...
          className="bg-green-900 hover:bg-green-950 text-white font-bold py-2 px-4 rounded"
          disabled={
            currentPage === Math.ceil(filteredTours.length / toursPerPage)
          }
        >
          Trang sau
        </button>
