...
export default function Trung() {
  ...
  useEffect(() => {
    const fetchTours = async () => {
      try {
        ...
        setFilteredTours(response.data.filter((tour) => tour.idTour.startsWith("MT")));
      } catch (error) {
  ...
  const handleSearch = () => {
    const filtered = tours.filter((tour) => {
      ...
      return matchesSearchTerm && matchesStartDate;
    });
      ...
      <div className="mr-10">
        <form className="flex justify-end mt-8 space-x-4">
          <input
            ...
            className="p-2 border border-green-900 rounded-lg w-60 focus:outline-none focus:ring-2 focus:ring-green-500"
          />
          <button
